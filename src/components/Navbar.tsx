    ...
    >
      <div className="max-w-[1920px] mx-auto px-6 md:px-12 flex items-center justify-between relative">
        ...
        <div className="flex items-center gap-12">
          <Link to="/" className="font-sans text-2xl tracking-[0.15em] font-semibold uppercase shrink-0">
            Soho Home
          </Link>
          ...
          <div className="hidden xl:flex items-center gap-6 text-[13px] font-medium tracking-wide">
            {NAV_LINKS.map((link) => (
              <Link 
                key={link} 
                to={`/collections/${link.toLowerCase().replace(/ & /g, '-').replace(/ /g, '-')}`}
                className={cn(
                  "transition-opacity whitespace-nowrap pb-1 border-b-2 border-transparent",
                  activeMenu === link ? "border-stone-900 opacity-100" : "hover:opacity-70"
                )}
            ...
            <a href="#interior-design" className="hover:opacity-70 transition-opacity whitespace-nowrap">Interior Design</a>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          <div className="hidden md:flex items-center gap-5">
            ...
            <button className="hover:opacity-70 transition-opacity"><Heart size={20} strokeWidth={1.5} /></button>
            <button 
      ...
      <div className={cn(
        ...
        <div className="p-6 pt-16 flex justify-between items-center border-b border-stone-200">
          <span className="font-sans text-xl tracking-[0.15em] font-semibold uppercase">Menu</span>
